...
def trackMultipleObjects():
    ...
    while True:
        ...
        if not (frameCounter % 10):
            gray = cv2.cvColor(image, cv2.COLOR_BG2GRAY)
            ...
            for (_x, _y, _w, _h) in cars:
                ...
                for carID in carTracker.keys():
                    ...
                    if ((t_x <= x_bar <= (t_x + t_w)) and (t_y <= y_bar <= (t_y + t_h)) and (x <= t_x_bar <= (x + w)) (y <= t_y_bar <= (y + h))):
                        matchCarID = carID
            ...
            if frameCounter % 1 == 0:
                ...
                if [x1, y1, w1, h1] != [x2, y2, w2, h2]:
                    ...
                    if speed[i] != None and y >= 100:
                        cv2.putText(resultImage, str(int(speed[i]) + "km/h", (int(x1 + w1/2), int(y1-5)), cv2.FONT_HARSHEY_SIMPLEX, 0.75, (0, 0, 100) ,2))
